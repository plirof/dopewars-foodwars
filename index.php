...
{
	echo "<html><head><title>dwars</title>";
	print_css( );
	echo "</head><body><center><h3>dwars - high scores</h3>";
	...
	switch ( $action )
	{
		case "login":
				...
				else if ( 1 < strlen($name) && 1 < strlen($password) )
				{
					...
					if ( $new )
					{
						...
						else
						{
							...
							$player['guns']				= 0;
							$player['helpboyes']			= 2;
							$player['space']			= 20 + $player['helpboyes']*10;
							$player['held']				= 0;
							$player['life']				= 100;
							$player['guns']				= Array( );
							$player['vegetables']			= Array( );
			...
			if ( !isset($_SESSION['uid']) || !isset($_SESSION['player']) )
			{
				...
				echo "<p>language: ";
				echo "<select name=lang><option value=EN-VEGET>English Vegetable Edition";
				echo "</select></p>";
		...
		default:
			...
			echo "<h2>dwars</h2>";
			echo "<p>(deal vegetables to make lots and lots of money)</p>";
...
{
	//print_r( $_SESSION );
	echo "fout... <a href='?logout=1'>Index</a>";
...
if ( isset($_GET['l']) && 0 <= $_GET['l'] && ( !isset($player['prison']) || !$player['prison'] ) && ( !isset($player['fight']) || !$player['fight'] ) )
{
	...
	echo "<html>".EOL.EOL."<head>".EOL."<title>dwars</title>".EOL;
...
if ( isset($player['destination']) && $player['destination'] != "" && $player['location']!=$player['destination'] && isset($player['travel']) && $player['travel'] )
{
			...
			else
			{
				...
				if ( $opponent["opponent"] )
				{
					if ( isset($_get['action']) && $_get['action'] == "fight" && !$player["reloading"])
					{
						...
						if ($damage >=  5 - $opponent["helpboyes"]/2)
						{

							if (mt_rand(0,$opponent["helpboyes"]))
							{
								$opponent = lose_helpboy($opponent);
								printf ($str["youkilledhelpboy"]. "<br>", htmlentities($opponent["name"]));
								$opponent["fighthistory"][] = "helpboykilled";
								$player["reloading"] = 1;
							}
							else
							{
					...
					{
						if ( (mt_rand(0,100) > 20 + $opponent["helpboyes"]* 7) || end($player["fighthistory"]) ==  "op_cantescape")
						{
							echo $str['escaped'];
		...
		else if ( isset($_GET['action']) && $_GET['action'] )
		{
			...
			if ( $_GET['action'] == "bribe")
			{
				if ( ( !isset($player['fightreport']['shots']) || $player['fightreport']['shots'] == 0 ) && $player['cash'] >= $player['cops']*20000)
				{
					...
					$player["fight"] = 0;
					reset ($player["vegetables"]);
					while (list ($key, $val) = each($player["vegetables"]))
					{
						$foo = round($val/2.0);
						$player['space'] += $foo;
						$player['vegetables'][$key] -= $foo;
					}
				}
				else
				{
			...
			if ( $_GET['action'] == "surrender")
			{
				...
				$prisontime += array_sum($player['vegetables'])/10;

				$player["prison"] = ceil(time() / 3600 + $prisontime) * 3600;
				$player["guns"] = array();
				$player["space"] += array_sum($player["vegetables"]);
				$player["vegetables"] = array();
			...
			else
			{
				...
				{
					if (mt_rand(0,$player["helpboyes"])) {
						$player = lose_helpboy($player);
						if ( !isset($player['fightreport']['helpboylost']) || 1 > $player['fightreport']['helpboylost'] )
						{
							$player['fightreport']['helpboylost']=1;
						}
						else
						{
							$player['fightreport']['helpboylost']++;
						}
						echo $str["helpboykilled"];
					} else {
						$damage = 1;
	...
	else	// NOT FIGHTING COPS OR PLAYERS
	{
		...
		else
		{
			...
			if ($r < 5)
			{
				...
				if ( 0 < array_sum($player["vegetables"]) && $r < 2)
				{
					// LOSES vegetables IN CHASE
					reset ($player["vegetables"]);
					while (list($key, $val) = each($player["vegetables"]))
					{
						if ($val)
						{
							$player["space"] += $player["vegetables"][$key];
							$player["vegetables"][$key] = 0;
							$player["drugprices"][$key] = 0;
							printf($str["lostvegetables"], $vegetables[$key]["name"]);
							break;
						}
					}	
				}
				...
				{
					// FINDS vegetables ON STREET
					$drug = mt_rand(0, count($vegetables)-1);
					$quantity = mt_rand(1, min(20, $player["space"]));
					$player["space"] -= $quantity;

					if ( !isset($player["vegetables"][$drug]) )		$player["vegetables"][$drug] = 0;
					if ( !isset($player["drugprices"][$drug]) )	$player["drugprices"][$drug] = 0;

					$drugamount = round( $player["vegetables"][$drug] * $player["drugprices"][$drug] );
					$player["vegetables"][$drug] += $quantity;
					$player["drugprices"][$drug] = round($drugamount/$player["vegetables"][$drug]);
					printf($str["foundbody"], $quantity, $vegetables[$drug]["name"]);
				}
				else
				{
					// GETS MUGGED
			...
			else if ($r < 10 && $player['helpboyes'] > 7)
			{
				$player = lose_helpboy($player);
				echo "<h3>".$str['onthemove']." ".$places[$player['destination']]."</h3>";
				echo $str['helpboygone'];
				echo "<form><input type=submit value=\"".$str['continue']."\"></form>";
				echo "</body></html>";
				$player['travel'] = 0;
				save_exit();
	...
	if ( isset($player['fight']) && $player['fight'] )
	{
		...
		{
			printf("<p>".$str['op_status']."</p>", htmlentities($opponent["name"]), $opponent["helpboyes"], array_sum($opponent["guns"]), $opponent["life"]);
		}
		echo "</body></html>";
		save_exit();
	}
		...
		while (list($key, $snitchreport) = each($player["snitchreport"])) {
			...
			if ($snitchreport["life"] != "" && $snitchreport["life"] <= 0) {
				...
				if ($snitchreport["arrested"]) {
					...
					if ($snitchreport["helpboylost"] == 1) {
						printf($str["d_helpboy"] . "<br>");
					} else if ($snitchreport["helpboylost"] > 1) {
						printf($str["d_helpboyes"] . "<br>", $snitchreport["helpboylost"]);
					}
				}
			}
...
if ($player['destination']!="" && $player['location']!=$player['destination'])
{
	...
	{
		$prices[$i] = mt_rand($vegetables[$i]['min'], $vegetables[$i]['max']);
		$foo = mt_rand(0,30);
		if ($foo == 1)
		{
			// higher
	...
	$drugcnt = mt_rand(count($vegetables)/2, count($vegetables));

	while ($drugcnt < count($prices))
	{
		unset($prices[mt_rand(0,count($vegetables))]);
	}
	...
	foreach ( $player['vegetables'] AS $key => $amount )
	{
		if ( 0 >= $amount )
		{
			unset( $player['drugprices'][$key] );
			unset( $player['vegetables'][$key] );
		}
	}
}
...
{
	$price = $helpboyactions[$_GET['activity']]['price'];
	if ( $price )
	{
		if ($player['cash'] >= $price)
		{
			switch ($_GET['activity'])
			{
				...
				case 3:
					...
					if ( $player['helpboyes'] >= MAX_helpboyES )
					{
						echo $str['maxhelpboy'];
					}
					else
...
if ( isset($_GET['action']) && ($_GET['action'] == "buy" || $_GET['action'] == "sell") && isset($_GET['quantity']) && 0 <= $_GET['quantity'] )
{
		...
		if ( $player["prices"][$_GET['drug']] || $_GET['action'] != "buy" )
		{
			if ($_GET['action'] == "buy")
			{
				...
				else if ( 0 < $_GET['quantity'] )
				{
					...
					if ( !isset($player['vegetables'][$_GET['drug']]) )		 	$player['vegetables'][$_GET['drug']] = 0;
					if ( !isset($player['drugprices'][$_GET['drug']]) ) 	$player['drugprices'][$_GET['drug']] = 0;

					$drugamount = round( $player['vegetables'][$_GET['drug']] * $player['drugprices'][$_GET['drug']] );
					$drugamount += $realprice;

					$player['vegetables'][$_GET['drug']] += $_GET['quantity'];
					$player['drugprices'][$_GET['drug']] = round($drugamount / $player['vegetables'][$_GET['drug']]);
			...
			{
				// sell vegetables
				if ($_GET['quantity'] > $player['vegetables'][$_GET['drug']])
				{
					printf($str['nodrug'], $_GET['quantity'], $vegetables[$_GET['drug']]['name']) . "<br>";
				}
				else
				{
					$player['cash'] += $realprice;
	...
	else if ( isset($_GET['gun']) && $_GET['gun'] != '' && $_GET['quantity'] > 0 && $player['location'] == 5)
	{
		...
		if ( isset($_GET['action']) && $_GET['action'] == "buy" )
		{
			...
			else if ($_GET['quantity'] > $player["helpboyes"] + 2 - $totalguns)
			{
...
if ( isset($_GET['s']) && $_GET['s'] )
{
	switch ($player['location'])
	{
		...
		case 1:
			...
			echo $str['hirehelpboy'];
			echo "<br><br><select name=activity size=4 style=\"width:200px;\">";
			reset($helpboyactions);
			while (list($key, $val) = each($helpboyactions))
			{
...
else
{
	...
	{
		// echo "[optional - vegetables-msg]<br>";
		echo '<font class="vegetablesextremes">';
		while (list($drug, $val) = each($player["prices"]))
		{
			if ($val < $vegetables[$drug]['min'])
			{
				if ($vegetables[$drug]['minmsg'])
				{
					echo $vegetables[$drug]['minmsg']."<br>\n";
				}
				else
				{
					$foo = mt_rand(0, count($minmsgs)-1);
					printf($minmsgs[$foo]."<br>", $vegetables[$drug]['name']);
				}
			}
			if ($val > $vegetables[$drug]['max'])
			{
				if ($vegetables[$drug]['maxmsg'])
				{
					echo $vegetables[$drug]['maxmsg']."<br>\n";
				}
				else
				{
					$foo = mt_rand(0, count($maxmsgs)-1);
					printf($maxmsgs[$foo]."<br>", $vegetables[$drug]['name']);
				}
			}
		}
		echo '</font><br>'.EOL;
...
{
	global $str, $places, $special, $vegetables, $helpboyactions, $fight;
	global $language, $currency, $guns, $maxmsgs, $minmsgs;

	if ( isset($_SESSION['language']) && $_SESSION['language'] == "NL")
	{  	
		include_once "msg_files/msg_nl.php";  //  jon modification 20140308
	}
	else if ( isset($_SESSION['language']) && $_SESSION['language'] == "EN-VEGET")
	{
		include_once "msg_files/msg_vegetables_en.php";  //  jon modification 20140308
	}
	else
	{
		$language = "EN";
		...
		$str["hire"]	= "hire";
		$str["hirehelpboy"]	= "hire helpboy to / for:";
		$str["maxhelpboy"]	= "You can't hire more than 10 helpboyes to carry vegetables.";
		...
		$str["helpboykilled"]	= "One of your helpboyes got killed.";
		...
		$str["lostvegetables"]	= "They chased you! You lost the %s.";
		$str["foundbody"]	= "You find the dead body of a helpboy with %s x %s.";
		$str["dead"]		= "You're <b>dead</b>!";

		$str["invalidname"]	= "Invalid name";
		$str["name"]		= "name";
		$str["hiresnitch"]	= "Hire a helpboy to tip off a dealer to the cops.<br>The cops will attack that dealer.<br>Later you will be informed on the encounter.";
		$str["snitchhired"]	= "%s is being tipped of.";
		$str["snitched"]	= "You were tipped off by %s.";
		$str["report"]		= "<b>Snitch report</b>";

		$str["spyreport"]	= "<p><b>Spy report</b></p>Dealer %s is located in %s,<br>has \$ %s in cash, \$ %s in the bank and a debt of \$ %s.<br>%s has %s helpboyes, %s guns and space left for %s vegetables.<br>Health is %s%%.";
		$str["hirespy"]		= "Hire a helpboy to find out the status of another dealer. You receive an answer immediately.";
		...
		$str["d_helpboy"]		= "One helpboy got killed.";
		$str["d_helpboyes"]	= "%s helpboyes got killed.";
		...
		$str["youkilledhelpboy"]	= "You wasted one of the helpboyes of %s.";
		$str["youkilledopponent"]	= "You killed %s. You find \$ %s on him.";
		$str["youshotopponent"]	   = "You hit %s.";
		$str["opponentdead"]	= "%s is dead.";

		$str["helpboygone"]	= "One of your helpboyes ran away.";

		$str["encounter"]	= "You run into %s, what do you do?";

		$str["op_status"]	= "Status of %s:<br>helpboyes: %s<br>weapons: %s<br>health: %s%%";
...
function check_max()
{
	...
	if ($count >= MAX_USERS)
	{
		echo "<html><title>dwars</title><head></head><body><h2>dwars - login</h2>";
...
function save_exit()
{
	...
	reset($player['vegetables']);
	while (list($drug, $val) = each($player['vegetables']))
	{
...
function lose_helpboy( $player )
{
	global $vegetables, $guns;

	// subtract proportional amount of vegetables
	reset ($player['vegetables']);
	while (list($key, $val) = each($player['vegetables'])) {
		$num = round($val/($player['helpboyes'] + 2));
		$player['vegetables'][$key] -= $num;
		$player['space'] += $num;
	}

	// remove guns
	if (round(array_sum($player['guns'])/($player['helpboyes'] + 2))) {
		reset ($player['guns']);
...
function printmenu( $side = NULL )
{
	...
	if ($side == "right")
	{
		...
		echo '<tr><td class="menu_stats" align="right">'.$str['helpboyes'].':</td><td class="menu_stats">'.$player['helpboyes'].'</td></tr>';
