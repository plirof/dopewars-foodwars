...
if ( isset($_GET['action']) && "overnight" == $_GET['action'] && $_GET['pwd'] == "aarde")
{
	...
	$posql->query("UPDATE dopewars SET gameoff='1';");
	...
	$posql->query("UPDATE dopewars SET gameoff='0';"); 
...
function check_max()
{
	...
	$result_query_2215=$posql->query("SELECT id FROM dopewars WHERE (now()-date)<60");
	//$count = mysql_num_rows(mysql_query("SELECT id FROM dopewars WHERE (now()-date)<60"));
	$count = $result_query_2215->rowCount;
	if ($count >= MAX_USERS)
	{
